...
# get uint8 image of size HxWxn_channles (RGB)
# ----------------------------------------
def imread_uint_resize(path, n_channels, size):
    # input : path
    # output: HxWx3(RGB or GGG), or HxWx1 (G)
    if n_channels == 1:
        img = cv2.imread(path, 0)  # cv2.IMREAD_GRAYSCALE
...
# ----------------------------------------
# get uint8 images of size HxWxn_channles (RGB)
# ----------------------------------------
...
# get uint8 images of size HxWxn_channles (RGB)
# ----------------------------------------
def imreads_uint_resize(paths, n_channels, size):
    #  input: paths
    # output: [HxWx3(RGB or GGG)], or [HxWx1 (G)]
    imgs = []
    for idx, path in enumerate(paths):
        img = imread_uint_resize(path, n_channels, size)
        imgs.append(img)
    return imgs
